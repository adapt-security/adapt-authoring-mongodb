...
class MongoDBModule extends AbstractModule {
  ...
  async connect() {
    try {
      ...
      this.log('error', `failed to connect to DB, ${e.message}`);
      throw this.app.errors.MONGO_CONN_FAILED
        .setData({ 
          uri: this.getConfig('connectionUri'),
          error: e.message 
        });
    }
  }
  ...
  getError(collectionName, action, error) {
    ...
    if(e.code === 66) e = this.app.errors.MONGO_IMMUTABLE_FIELD;
    else if(e.code === 11000) e = this.app.errors.MONGO_DUPL_INDEX;
