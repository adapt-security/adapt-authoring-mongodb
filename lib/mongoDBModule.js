...
class MongoDBModule extends AbstractModule {
  ...
  */
  ...
  constructor(...args) {
    ...
    /**
    * Hook invoked when a new document is inserted
    * @type {Hook}
    */
    this.insertHook = new Hook({ type: Hook.Types.Series, mutable: true });
    /**
    * Hook invoked when a document is replaced
    * @type {Hook}
    */
    this.replaceHook = new Hook({ type: Hook.Types.Series, mutable: true });
  ...
  */
  async insert(collectionName, data, options={}) {
    ...
    let results;
    try {
      const insertHookData = await this.insertHook.invoke(collectionName, data);
      results = (await this.getCollection(collectionName).insertOne(insertHookData));
    } catch(e) {
  ...
  */
  async replace(collectionName, query, data, options={}) {
    ...
    let result;
    try {
      const replaceHookData = await this.replaceHook.invoke(collectionName, data);
      result = await this.getCollection(collectionName).findOneAndReplace(query, replaceHookData, options);
    } catch(e) {
