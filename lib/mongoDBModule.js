...
class MongoDBModule extends AbstractModule {
  ...
  constructor(...args) {
    super(...args);
    console.log(this.getConfig('connectionUri'));
    /** @ignore */ this.client = new MongoClient(this.getConfig('connectionUri'), {
  ...
  async init() {
    try {
      ...
      this.log('error', `Failed to initialise ${this.name}, ${e}`);
    }
    this.setReady();
  }
  /**
  ...
  async connect(options = {}) {
    ...
    this.log('info', this.app.lang.t('info.connected', { uri: this.getConfig('connectionUri') }));
  }
  /**
