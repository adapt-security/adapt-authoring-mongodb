...
class MongoDBModule extends AbstractModule {
  ...
  */
  async update(collectionName, query, data, options) {
    ...
    try {
      const [existingDoc] = await this.find(collectionName, query);
      if(!existingDoc) throw new Error('no matching document found');

      const { value } = await this.getCollection(collectionName).findOneAndUpdate(query, data, opts);
      this.emit('update', collectionName, existingDoc, value);
      return value;
    } catch(e) {
  ...
  */
  async replace(collectionName, query, data, options) {
    ...
    try {
      ...
      this.emit('replace', collectionName, existingDoc, result.value);
      return result.value;
    } catch(e) {
  ...
  */
  async delete(collectionName, query, options) {
    ...
    try {
      ...
      this.emit('delete', collectionName, existingDoc);
      return value;
    } catch(e) {
