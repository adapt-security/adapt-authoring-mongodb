...
class MongoDBModule extends AbstractModule {
  ...
  */
  async insert(collectionName, data, options = {}) {
    ...
    try {
      ...
      throw new Error(this.t('error.insertdocs'));
    }
  }
  /**
  ...
  */
  async find(collectionName, query, options = {}) {
    ...
    try {
      ...
      throw new Error(`${this.t('error.finddocs')}, ${e.message}`);
    }
  }
  /**
  ...
  */
  async update(collectionName, query, data, options = { returnOriginal: false }) {
    ...
    try {
      const result = await this.getCollection(collectionName).findOneAndUpdate(query, data, options);
      if(result.value) return result.value;
    } catch(e) {
      throw new Error(`${this.t('error.updatedocs')}, ${e.message}`);
    }
  }
  /**
  ...
  */
  async replace(collectionName, query, data, options = { returnOriginal: false }) {
    this.ObjectId.parseParamIds(query, data);
    try {
      const result = await this.getCollection(collectionName).findOneAndReplace(query, data, options);
      if(result.value) return result.value;
    } catch(e) {
      throw new Error(`${this.t('error.replacedocs')}, ${e.message}`);
    }
  }
  /**
  ...
  */
  async delete(collectionName, query, options = {}) {
    this.ObjectId.parseParamIds(query);
    try {
      const result = await this.getCollection(collectionName).findOneAndDelete(query, options);
      if(result.value) return result.value;
    } catch(e) {
      throw new Error(`${this.t('error.deletedocs')}, ${e.message}`);
    }
  }
  /**
