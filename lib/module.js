...
class MongoDBModule extends DataStore {
  ...
  */
  ...
  connect(options = {}) {
    return new Promise((resolve, reject) => {
      ...
      */
      this.connection = mongoose.createConnection(this.connectionURI, { useNewUrlParser: true, ...options });
  ...
  */
  getModel(name) {
    ...
    if(!m) {
      throw this.formatError(this.app.lang.t('error.getmodel', { model: String(name) }), 'error.unknownmodel', Responder.StatusCodes.Error.User);
    }
    return m;
  }
  /**
  ...
  */
  ...
  create(data) {
    return new Promise((resolve, reject) => {
      if(!data) {
        return reject(this.formatError(this.app.lang.t('error.nocreatedata'), 'error.createdocs', Responder.StatusCodes.Error.User));
      }
      try {
        ...
        return reject(this.formatError(e, this.app.lang.t('error.createdocs', Responder.StatusCodes.Error.User)));
      }
    });
  }
  /** @override*/
  retrieve(query) {
    return new Promise((resolve, reject) => {
      ...
      q.then(resolve).catch(error => reject(this.formatError(error.message, 'error.retrievedocs')));
    });
  }
  /** @override*/
  update(query, data) {
    return new Promise((resolve, reject) => {
      ...
      Model.updateMany(formatted.query, data, { runValidators: true, ...formatted.options })
        .then(resolve)
        .catch(error => reject(this.formatError(error.message, 'error.updatedocs')));
    });
  }
  /** @override*/
  delete(query) {
    return new Promise((resolve, reject) => {
      ...
      Model.deleteMany(formatted.query, error => {
        if(error) return reject(this.formatError(error.message, 'error.deletedocs'));
        resolve();
      });
    });
  }
