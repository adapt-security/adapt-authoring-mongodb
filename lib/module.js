...
class MongoDB extends DataStore {
  ...
  */
  constructor(...args) {
    ...
    /** @ignore */ this.connectionURI = `mongodb://localhost:27017/adapt-authoring-test`;
    /** @ignore */ this.models = {};
  }
  /**
  ...
  */
  connect(options = {}) {
    /** @ignore */ this.connection = mongoose.createConnection(this.connectionURI, options);

    this.connection
      .then(connection => this.log('info', `${this.readyState}`))
      .catch(error => this.log('error', `${errors.ConnectionFail}, ${error}`));
  ...
  */
  create(data) {
    return new Promise((resolve, reject) => {
      ...
      (new Model(data)).save().then(resolve).catch(error => reject(new Error(`${errors.CreateFail}, ${error.message}`)));
    });
  }
  ...
  */
  update(query, data) {
    return new Promise((resolve, reject) => {
      ...
      try {
        formatted = this.formatQuery(query);
        q = this.getModel(query.type).update(formatted.query, data, formatted.options);
      } catch(e) {
        reject(e);
      }
      q.then(resolve).catch(error => reject(new Error(`${errors.UpdateFail}, ${error.message}`)));
    });
  }
  /**
