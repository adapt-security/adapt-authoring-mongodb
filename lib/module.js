...
class MongoDB extends DataStore {
  ...
  */
  constructor(app, config) {
    ...
    /** @ignore */ this.connectionURI = `mongodb://${config.DB_HOST}:${config.DB_PORT}/${config.DB_NAME}`;
    /** @ignore */ this.models = {};
  }
  /**
  ...
  formatValidationError(error, errorPrefix) {
    ...
    Object.entries(error.errors).forEach(([key,{ kind, path, value }]) => {
      ...
      switch(kind) {
        ...
        case 'user defined': m = `${path} ${errors.IsRequired}`; break;
        default: console.log(kind, path, value); break;
      }
      e.addError(m);
    });
    return e;
