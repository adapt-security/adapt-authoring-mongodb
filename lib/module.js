const { DataStore, DataStoreQuery, DataValidationError } = require('adapt-authoring-core');
...
class MongoDB extends DataStore {
  ...
  */
  addModel(data) {
    ...
    try {
      ...
      this.log('debug', this.app.lang.t('success.addmodel', { model: data.name }));
      return m;
    } catch(e) {
  ...
  */
  getModel(name) {
    ...
    if(!m) {
      throw new Error(`${this.app.lang.t('error.getmodel', { model: data.name })}, ${error.UnknownModel}`);
    }
    return m;
  }
  /**
  ...
  */
  ...
  retrieve(query) {
    return new Promise((resolve, reject) => {
      ...
      q.then(resolve).catch(error => reject(new Error(`${this.app.lang.t('error.retrieve')}, ${error.message}`)));
    });
  }
  /** @override*/
  update(query, data) {
