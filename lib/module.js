...
class MongoDB extends DataStore {
  ...
  * @param {MongoModel} model Model instance
  ...
  */
  addModel(model) {
    ...
    if(!model instanceof MongoModel) {
      return this.log(`warn, ${errors.AddModelFail}, not a valid MongoModel, ${e}`);
    }
    try {
      this.models[model.name] = model;
      model.compiled = this.connection.model(model.name, model.schema);
    } catch(e) {
      return this.log(`warn, ${errors.AddModelFail}, ${errors.InvalidSchema}, ${e}`);
    }
    return this.getModel(model.name);
  }
  /**
