...
class MongoDB extends DataStore {
  ...
  */
  addModel(name, schema) {
    console.log('MongoDB#addModel:', name);
    if(!this.connection) {
  ...
  */
  retrieve(query) {
    return new Promise((resolve, reject) => {
      const Model = instance.getModel(query.type);

      if(!model) {
        return reject(`${usererrors.RetrieveError}, ${errors.UnknownModel}`);
      }
      const formatted = this.formatQuery(query);

      Model.find(formatted.query, formatted.options)
        .then(resolve)
        .catch(error => reject(`${usererrors.RetrieveError}, ${error.message}`));
    });
  }
}
